...
import deepanshu.example.com.moviez.databinding.ItemSimilarMovieBinding
import deepanshu.example.com.moviez.modules.movie.api.response.Movie

class SimilarMoviesAdapter(private val movies :List<Movie>, listener: OnItemClickListener<Movie>):BaseRecyclerviewAdapter<Movie>(listener) {

    override fun getItemForPosition(position: Int): Movie = movies[position]

    override fun getLayoutIdForPosition(position: Int): Int = R.layout.item_similar_movie

    override fun getItemCount(): Int = movies.size

    override fun onBindViewHolder(holder: BaseViewHolder<Movie>, position: Int) {
        super.onBindViewHolder(holder, position)
        val binding:ItemSimilarMovieBinding = holder.getBinding() as ItemSimilarMovieBinding
